...
function PlotHydrogenMolecularOrbital_sp3
%--------------------------------------------------------------------------
...
cons.cutoff     = 1.2 * 10^(28); % electron density cutoff (where to put isosurface)
...
WaveFn_sp3_1 = 0.5 * (WaveFn_2s + WaveFn_2px + WaveFn_2py + WaveFn_2pz);
WaveFn_sp3_2 = 0.5 * (WaveFn_2s + WaveFn_2px - WaveFn_2py - WaveFn_2pz);
WaveFn_sp3_3 = 0.5 * (WaveFn_2s - WaveFn_2px + WaveFn_2py - WaveFn_2pz);
WaveFn_sp3_4 = 0.5 * (WaveFn_2s - WaveFn_2px - WaveFn_2py + WaveFn_2pz);

subplot(2,2,1);
WaveFn = WaveFn_sp3_1;
cons.orbitName   = 'sp3_1';
cons.sliceStyle2 = false;
PlotCrossSectionIsosurface(xSpace, ySpace, zSpace, WaveFn, cons);

subplot(2,2,2);
WaveFn = WaveFn_sp3_2;
cons.orbitName   = 'sp3_2';
cons.sliceStyle2 = false;
PlotCrossSectionIsosurface(xSpace, ySpace, zSpace, WaveFn, cons);

subplot(2,2,3);
WaveFn = WaveFn_sp3_3;
cons.orbitName   = 'sp3_3';
cons.sliceStyle2 = true;
PlotCrossSectionIsosurface(xSpace, ySpace, zSpace, WaveFn, cons);

subplot(2,2,4);
WaveFn = WaveFn_sp3_4;
cons.orbitName   = 'sp3_4';
cons.sliceStyle2 = true;
PlotCrossSectionIsosurface(xSpace, ySpace, zSpace, WaveFn, cons);
end
