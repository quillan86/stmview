...
function PlotHydrogenMolecularOrbital_3p
%--------------------------------------------------------------------------
...
cons.cutoff     = 0.75 * 10^(26); % electron density cutoff (where to put isosurface)
...
PlotCrossSectionIsosurface(xSpace, ySpace, zSpace, WaveFn_2px, cons);

subplot(2,2,2);
m = 1;
WaveFn_2py = psiCalculation(n,l,m,r,theta,phi,cons);
cons.orbitName   = orbitNameIs(n,l,m);
PlotCrossSectionIsosurface(xSpace, ySpace, zSpace, WaveFn_2py, cons);
...
PlotCrossSectionIsosurface(xSpace, ySpace, zSpace, WaveFn_2pz, cons);

subplot(2,2,4);
WaveFn = WaveFn_2px + WaveFn_2py + WaveFn_2pz;
cons.orbitName   = '2px + 2py + 2pz';
