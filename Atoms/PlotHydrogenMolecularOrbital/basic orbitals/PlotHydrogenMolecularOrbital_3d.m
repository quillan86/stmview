...
function PlotHydrogenMolecularOrbital_3d
%--------------------------------------------------------------------------
% define constants and orbital shape

% quantum numbers
n = 3;           % principal quantum number (shell #)
l = 2;           % orbital quantum number (subshell type: s, p, d, f, etc.)
...
PlotCrossSectionIsosurface(xSpace, ySpace, zSpace, WaveFn_3dm2, cons);
fprintf('done with plot 1\n');

subplot(2,3,2);
m = -1;
WaveFn_3dm1 = psiCalculation(n,l,m,r,theta,phi,cons);
cons.orbitName   = orbitNameIs(n,l,m);
cons.sliceStyle2 = true;
PlotCrossSectionIsosurface(xSpace, ySpace, zSpace, WaveFn_3dm1, cons);
...
PlotCrossSectionIsosurface(xSpace, ySpace, zSpace, WaveFn_3d0, cons);
fprintf('done with plot 3\n');

subplot(2,3,4);
m = 1;
WaveFn_3d1 = psiCalculation(n,l,m,r,theta,phi,cons);
cons.orbitName   = orbitNameIs(n,l,m);
PlotCrossSectionIsosurface(xSpace, ySpace, zSpace, WaveFn_3d1, cons);
fprintf('done with plot 4\n');

subplot(2,3,5);
m = 2;
WaveFn_3d2 = psiCalculation(n,l,m,r,theta,phi,cons);
cons.orbitName   = orbitNameIs(n,l,m);
PlotCrossSectionIsosurface(xSpace, ySpace, zSpace, WaveFn_3d2, cons);
fprintf('done with plot 5\n');

subplot(2,3,6);
WaveFn = WaveFn_3dm2 + WaveFn_3dm1 + WaveFn_3d0 + WaveFn_3d1 + WaveFn_3d2;
cons.orbitName   = '3d summation';
