...
function [X, Y, Z] = cylinder2P_mod(R, N,r1,r2)
    ...
    theta = linspace(pi/4,3*pi/4,N); theta = theta-pi/2;
    ...
    R2=rand(1,3);
    R2 = [1 1 1];%linear independent vector (of v)
    ...
    z = abs([0.8*cos(2*(theta+pi/4)) + 0.15*cos(6*(theta+pi/4))]);
    time=linspace(0,1,m);    
    for j = 1 : m
        if j == m
            for k = 1:N
                t=time(j);
                X(j, k) = r1x+(r2x-r1x)*t+R(j)*cos(theta(k))*x2x+R(j)*sin(theta(k))*x3x;
                Y(j, k) = r1y+(r2y-r1y)*t+R(j)*cos(theta(k))*x2y+R(j)*sin(theta(k))*x3y;                
                if k < 20 || k > (N-20)
                    Z(j, k) = z(k)*(r1z+(r2z-r1z)*t+R(j)*cos(theta(k))*x2z+R(j)*sin(theta(k))*x3z);
                else
                    Z(j, k) = z(k)*(r1z+(r2z-r1z)*t+R(j)*cos(theta(k))*x2z+R(j)*sin(theta(k))*x3z);
                end
            end
