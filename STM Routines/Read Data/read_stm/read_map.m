...
digits(7);
    ...
    if type==0 || type ==1
        ...
        data.rx = data.r; % add coordinate for x and y directions so to 
        data.ry = data.r; % track of relative positions during crops etc.
    ...
    elseif type == 2
       % dist=hs.xdist;
        ...
        data.r = linspace(0,hs.xdist,s)'; % want to eventually get rid of this
        data.rx = data.r; % add coordinate for x and y directions so to 
        data.ry = data.r; % track of relative positions during crops etc.
        data.info = hs;
        data.var = var;
