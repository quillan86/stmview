function DW_directional_dialogue(data)
...
%form factors
fh=figure('Name', 'Form Factor DW Directionality - DW Q vectors + Filter Width',...
        'units','normalized', ...
...
function OK_Callback(hObject,eventdata)
    ...
    d_opt = get(d_check,'Val')
    s_opt = get(s_check,'Val')
    sp_opt = get(sp_check,'Val')
    DW_directional(q_px,filt_w,data,[s_opt sp_opt d_opt]);
end
