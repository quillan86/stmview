...
'Parent',fh,...
'Units','characters',...
'Position',[7 8.05000000000001 77 39]);

phase_fixed = axes(...
'Parent',fh,...
'Units','characters',...
'Position',[91.2857142857143 8.05 77 39]);
...
'Clipping','on',...
'Position',[7 1.3 17.7142857142857 4.00]);

button_1 = uicontrol(...
'Parent',dir_button_group,...
'Units','characters',...
'Position',[1.57142857142857 1.45 13 1.45],...
'String','Direction 1',...
'Style','radiobutton');
...
'Parent',fh,...
'Units','characters',...
'Position',[7 47.45 19.4285714285714 1.0],...
...
'Parent',fh,...
'Units','characters',...
'Position',[91.2857142857143 47.45 19.4285714285714 1.0],...
...
'Parent',fh,...
'Units','characters',...
'Position',[35.5714285714286 6.5 28.7142857142857 1.0],...
'Style','text');
...
'Units','characters',...
'BackgroundColor',[1 1 1],...
'Position',[91.2857142857143 5.05 11.4285714285714 1.35],...
'String', '0',...
'Style','edit');
...
'Parent',fh,...
'Units','characters',...
'Position',[91.2857142857143 6.5 9 0.9],...
'String','Tolerance:',...
'Style','text');
...
'Parent',fh,...
'Units','characters',...
'Position',[106.7142857142857 5.15 14.4285714285714 1.2],...
'String','Adjust Phase',...
...
'Parent',fh,...
'Units','characters',...
'Position',[90.7142857142857 3.25 14.4285714285714 1.2],...
'String','Add 2\pi',...
'Callback',{@add_subt_pi_Callback,1});

subt_pi = uicontrol(...
'Parent',fh,...
'Units','characters',...
'Position',[90.7142857142857 1.35 14.4285714285714 1.2],...
'String','Subtract 2\pi',...
'Callback',{@add_subt_pi_Callback,-1});

undo_button = uicontrol(...
'Parent',fh,...
'Units','characters',...
'Position',[106.7142857142857 3.25 14.4285714285714 1.2],...
'String','Undo Last',...
...
'Parent',fh,...
'Units','characters',...
'Position',[106.7142857142857 1.35 14.4285714285714 1.2],...
'String','Undo All',...
...
'Parent',fh,...
'Units','characters',...
'Position',[124.142857142857 4.85000000000001 14.4285714285714 1.85],...
'String','DONE',...
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Callback Functions %%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    function adjust_phase_Callback(hObject,eventdata)
        ...
        %adj_phase_routine(x1,y1,x2,y2,sel,fh,phase_fixed,tol_val,color_map)
        adj_phase_routine(x1,y1,x2,y2,sel,fh,phase_fixed,tol_val,color_map)
    end

    function add_subt_pi_Callback(hObject,eventdata,sgn_pi)
        x1 = str2num(get(st_X_edit,'String'));
        y1 = str2num(get(st_Y_edit,'String'));
        x2 = str2num(get(end_X_edit,'String'));
        y2 = str2num(get(end_Y_edit,'String'));
        ...
        phase_new = get(fh,'UserData');
       if sel == 1
            tmp = phase_new.theta1;
            tmp(y1:y2,x1:x2) = tmp(y1:y2,x1:x2) + sgn_pi*2*pi;
            phase_new.theta1_old = phase_new.theta1; %for UNDO
            phase_new.theta1 = tmp;
            
        elseif sel == 2
            tmp = phase_new.theta2;
            tmp(y1:y2,x1:x2) = tmp(y1:y2,x1:x2) + sgn_pi*2*pi;
            phase_new.theta2_old = phase_new.theta2; %for UNDO
            phase_new.theta2 = tmp;
       end
        set(fh,'UserData',phase_new);                 
        axes(phase_fixed);
        imagesc(tmp); axis off; colormap(color_map); 
    end    
...
function adj_phase_routine(x1,y1,x2,y2,selection,phase_data_handle,axis_handle,tol,c_map)               
