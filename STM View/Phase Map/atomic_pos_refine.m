...
% 140430 MHH  - Adjusted to give continuous values for atom position
%%%%%%%%

function pos_matrix = atomic_pos_refine(phase_map,phi_shift1,phi_shift2)

% topo phase reconstructed from phase_map (for BSCCO, gives Cu positions when phi_shift = 0)
...
%img_plot2(s_phase1 + phase_map.theta1); img_plot2(s_phase2 + phase_map.theta2);

% topo reconstructed from phase
p_lattice = sin(s_phase1) + sin(s_phase2);
%p_lattice2 = sin(s_phase1 + phase_map.theta1) + sin(s_phase2 +  phase_map.theta2);
%img_plot2(p_lattice1);
%img_plot2(p_lattice2)
...
% delete entries around the edges as the tend to be wrong
A(1:2,:) = 0;
A(end-1:end,:) = 0;
A(:,1:2) = 0;
A(:,end-1:end) = 0;
