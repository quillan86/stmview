function new_data = gap_scale_map(data,gap_map)
[nr nc nz] = size(data.map);
x = data.e;

zero_ind = find(data.e == 0);
if isempty(zero_ind)
    zero_ind = floor(nz/2) + 1;
end
sum_ind = 0;
count = 0;
...
x_rescale = e1:(sign*e_space):e2; x_rescale = x_rescale/gap_min/1;
nz_max = length(x_rescale);
zero_ind = find(x_rescale == 0);

new_map = zeros(nr,nc,nz_max);
h = waitbar(0,'Please wait...','Name','Gap Scaling in Progress');
for i = 1:nr
    %i
    for j= 1:nc
        gap_val = gap_map(i,j);
        %gap_val = find_gap_val(gap_map(i,j),x); %option if only want
        %values in energy vector
        Y = squeeze(squeeze(data.map(i,j,:)));
        if (gap_val ~= 0 && sum(isnan(Y)) == 0)
            %Y = squeeze(squeeze(data.map(i,j,:)));
            ...
            yi = interp1(x,Y,xi);
            %align the middle of new_map in the z-direciton with the middle
