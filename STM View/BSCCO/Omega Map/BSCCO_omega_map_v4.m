function [omap infl_map] = BSCCO_omega_map_v4(data,degree_poly,frac_peak,res,gap_index_map,dip_energy_map)
...
%gap_index = index_val_map(gap_map,e);
gap_index = gap_index_map;
%load_color
...
for i=1:nr      
    parfor j=1:nc              
        if gap_index(i,j) ~= 0 && dip_energy_map(i,j) ~=0          
            x = e(1:gap_index(i,j)); 
            y = squeeze(squeeze(map(i,j,1:gap_index(i,j)))); 
            l_y = length(y);
            peak_val = y(end);
            min_val_ind = find_nearest_index(x,dip_energy_map(i,j));
            ...
            %frac_peak_ind = find( (y>=frac_val)'.*(x<=x(min_val_ind)) ==
            %1,1); % for positive side
            
            kstart = 1; kend = l_y - frac_peak_ind + 1 ; % for pos side
            %kstart = 1; kend = frac_peak_ind + 1 ;
            count = 0;                
            while(count < 12)                      
