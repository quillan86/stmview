...
%              Q1 - [Y X] Modulation wavevector of SM in fourier space
%              upper half (in pixels)
%              Q2 - [Y X] Modulation wavevector of SM in fourier space
%              lower half (in pixels)
...
%
%%%%%%%
function phase = SM_phase(topo,Q1,Q2,filt_width1)

[nr nc] = size(topo);
% q values in fourier space from pixel values given in Q
q1 = 2*pi/nr*((Q1(1)-(nr/2+1)));
q2 = 2*pi/nr*((Q1(2)-(nc/2+1)));
...
phase = mod((q1*Y + q2*X) + theta - pi/2,2*pi); % shift by pi/2 to make top of SM at phi = 0
%img_plot2(phase)
