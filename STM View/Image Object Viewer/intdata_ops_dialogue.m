function intdata_ops_dialogue(current_obj_handle)

h1 = figure(...
'Units','characters',...
'Color',[0.941176470588235 0.941176470588235 0.941176470588235],...
'MenuBar','none',...
'Name','Inter-Data Operations',...
'NumberTitle','off',...
...
'Units','characters',...
'BackgroundColor',[1 1 1],...
'Position',[46.1428571428571 9 21.7142857142857 1.45],...
'String',{'[Single:Single]';'[Single:All]';'[All:Single]';'[All:All]'},...
'Style','popupmenu',...
'Value',1);
...
ops_type_title = uicontrol(...
'Parent',h1,...
'Units','characters',...
'BackgroundColor',[1 1 1],...
'Position',[46.1428571428571 10.7 21.7142857142857 1.2],...
'String',{  'Layer Options' },...
'Style','text');
...
function accept_Callback(hObject,eventdata)
    ...
    ops_val_table = {'+','-','*','/'};
    ops_type_val = get(ops_type,'Value');
    ...
    new_data = current_obj;
    
    if ops_type_val == 1
        layer_current_obj = get(img1_lyrs,'Value');
        layer_data2 = get(img2_lyrs,'Value');            
        new_data.map = map_ops(current_obj.map(:,:,layer_current_obj),data2.map(:,:,layer_data2),ops_val);
        new_data.e = current_obj.e(layer_current_obj);
        new_data.name = [current_obj.name '(' num2str(layer_current_obj) ')' ops_val_table{ops_val} data2.name '(' num2str(layer_data2) ')'];            
        
    elseif ops_type_val == 2
       layer_current_obj = get(img1_lyrs,'Value');
       new_data.name = [current_obj.name '(' num2str(layer_current_obj) ')' ops_val_table{ops_val} data2.name];            
       new_data.e = data2.e;
       new_data.map = map_ops(current_obj.map(:,:,layer_current_obj),data2.map,ops_val);
                 
    elseif ops_type_val == 3
        layer_data2 = get(img2_lyrs,'Value');
        new_data.name = [current_obj.name ops_val_table{ops_val} data2.name '(' num2str(layer_data2) ')'];            
        new_data.map = map_ops(current_obj.map,data2.map(:,:,layer_data2),ops_val);
        new_data.e = current_obj.e;
        
        
    elseif ops_type_val == 4
        new_data.name = [current_obj.name ops_val_table{ops_val} data2.name];            
        new_data.map = map_ops(current_obj.map,data2.map,ops_val);
        new_data.e = current_obj.e;
    end     
...
function new_map = map_ops(map1,map2,ops)
% ops - ADD(1), SUBTRACT(2), MULTIPLY(3), DIVIDE(3)
[nr1 nc1 nz1] = size(map1);
[nr2 nc2 nz2] = size(map2);

max_nz = max(nz1,nz2);

for k = 1:max_nz
    if ops == 1
        new_map(:,:,k) = map1(:,:,min(k,nz1)) + map2(:,:,min(k,nz2));
    elseif ops == 2
        new_map(:,:,k) = map1(:,:,min(k,nz1)) - map2(:,:,min(k,nz2));
    elseif ops == 3
        new_map(:,:,k) = map1(:,:,min(k,nz1)) .* map2(:,:,min(k,nz2));
