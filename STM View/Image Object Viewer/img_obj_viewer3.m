function h = img_obj_viewer3(in_data)
...
disp = uimenu('Label','Display');
    ...
    uimenu(disp,'Label','Show Average Spectrum + Variance','Callback',@avg_spect_var_Callback);
...
analysis = uimenu('Label','Analysis');
    ...
    bscco_anal = uimenu(analysis,'Label','BSCCO');
        ...
        uimenu(bscco_anal,'Label','Generate Cu Ox Oy index','Callback',@cuprate_atomic_site_gen_Callback);
        ...
        bscco_dw_anal = uimenu(bscco_anal,'Label','Sublattice & DW');
            ...
            uimenu(bscco_dw_anal,'Label','Generate FT of Ox+Oy,Ox-Oy','Callback',@OxOy_sum_diff_maps_Callback);
...
function save_zm_lyr_Callback(hObject,eventdata)  
    ...
    function cuprate_atomic_site_gen_Callback(hObject,eventdata)        
        data_loc = guidata(f);

        if ~isfield(data_loc,'phase_map')        
            display('No phase map information - Cannot proceeds');
            return;
        end
    ...
    function OxOy_sum_diff_maps_Callback(hObject,eventdata)
        ...
        img_obj_viewer2(sum_Re_FTOxOy);
        %colormap(cols);
        img_obj_viewer2(sum_Im_FTOxOy);
        %colormap(cols);
        img_obj_viewer2(diff_Re_FTOxOy);
        %colormap(cols);
        img_obj_viewer2(diff_Im_FTOxOy);
        %colormap(cols);
    end

    function OxOy_sum_diff_PSD_Callback(hObject,eventdata)
        [~,~,~, ~,psd_sum,psd_diff] = BSCCO_OxOy_sum_diff_map(guidata(f),1);
        img_obj_viewer2(psd_sum);
        img_obj_viewer2(psd_diff);
