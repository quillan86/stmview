function hist2d_dialogue(current_obj_handle)
...
'String',{'HIST[Single:Single]';'HIST[Single:All]';'HIST[All:Single]' },...
'Style','popupmenu',...
'Value',1);
...
function accept_Callback(hObject,eventdata)
    ...
    if ops_val == 1
        ...
        new_hist.name = [current_obj.name '_c' num2str(layer_current_obj) '_c' num2str(layer_data2) '_' data2.name];            
        [c MN xx yy] = hist2d_maps(current_obj.map(:,:,layer_current_obj),data2.map(:,:,layer_data2));
        new_hist.c = c; new_hist.histo = MN; new_hist.xbin = xx; new_hist.ybin = yy;
        img_plot2(MN);
    elseif ops_val == 2
       layer_current_obj = get(img1_lyrs,'Value');
       new_hist.name = [current_obj.name '_c' num2str(layer_current_obj) '_cn' data2.name];                  
       for k = 1:nz2
           [c MN xx yy] = hist2d_maps(current_obj.map(:,:,layer_current_obj),data2.map(:,:,k));
           new_hist.c(k) = c; 
           new_hist.histo(:,:,k) = MN; 
           new_hist.xbin(:,k) = squeeze(xx(1,:)); 
           new_hist.ybin(:,k) = squeeze(yy(:,1));
       end
       new_hist.e = data2.e;
    else
        layer_data2 = get(img2_lyrs,'Value');
        new_hist.name = [current_obj.name '_cn_c' num2str(layer_data2) '_' data2.name];        
        for k = 1:nz1
           [c MN xx yy] = hist2d_maps(current_obj.map(:,:,k),data2.map(:,:,layer_data2));
           new_hist.c(k) = c; 
           new_hist.histo(:,:,k) = MN;           
           new_hist.xbin(:,k) = squeeze(xx(1,:)); 
           new_hist.ybin(:,k) = squeeze(yy(:,1));
        end
        new_hist.e = current_obj.e;
    end     
    assignin('base','new_hist',new_hist);
