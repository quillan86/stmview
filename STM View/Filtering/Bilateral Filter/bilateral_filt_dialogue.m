function new_data = bilateral_filt_dialogue(data)
      ...
      % set(h,'Name','Bilateral Filter Progress');       
       ...
       parfor i = 1:nz           
           img = old_map(:,:,i);
           min_val =min(0, min(min(img)));
           img = img - min_val;
           max_val = max(max(abs(img)));
           new_map(:,:,i) = bilateral_filt(img/max_val,w,sigma)*max_val + min_val;
      ...
      % close(h);
       ...
       new_data.ops{end+1} = ['Bilateral Filter - filter width: ' answer{1} ', spatial SD: ' answer{2} ', intensity SD: ' answer{3}];
       IMG(new_data);      
